...
void GraphicsView::doMovePixmapToViewCenter()
{
    ...
    m_pixmapItem->setPos(rect.topLeft());
}
...
void GraphicsView::doSetToOptimalScale()
{
    ...
    QRectF maginedSceneRect = sceneRect().marginsRemoved(m_margin);
...
void GraphicsView::doUpdateLayout()
{
    ...
    {
        //qDebug()<< "resetPixmapScale";
        doResetPixmapScale();
    }
    else if (m_pixmapResizePolicy & OptimalSize)
    {
        //qDebug()<< "setToOptimalScale";
        doSetToOptimalScale();
    }

    if (m_pixmapResizePolicy & MoveToCenter)
    {
        //qDebug()<< "movePixmapToViewCenter";
